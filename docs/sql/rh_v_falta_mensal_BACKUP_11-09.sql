...
SELECT
    ...
    SUM(CASE WHEN FLG_DESC_SAL = 'S' THEN 1 ELSE 0 END) AS TOT_INJ,
    SUM(CASE WHEN FLG_DESC_SAL <> 'S' OR FLG_DESC_SAL IS NULL THEN 1 ELSE 0 END) AS TOT_JUS,
   /* FLAG mensal de desconto */
    CASE
        WHEN SUM(CASE WHEN FLG_DESC_SAL = 'S' THEN 1 ELSE 0 END) > 0
            THEN 'S'
        ELSE 'N'
        END AS FLG_DESC_SAL,
   CASE
