-- RH_V_RESUMO_ASSIDUIDADE — correcção de 11/09/2026
--
-- Defeito: TOTAL_FALTAS contava TODOS os dias sem registo em RH_T_FALTA, sem verificar se o dia
-- era sequer uma falta. Um dia normal de trabalho (FALTA=0, 8h feitas) não tem linha em
-- RH_T_FALTA, logo entrava na conta. Consequência: um colaborador com um mês de picagens normais
-- aparecia no ecrã de assiduidade com "N faltas injustificadas" e ESTADO='INJUSTIFICADA'.
-- Verificado em dev: o funcionário 958935 tinha 5 dias normais em Setembro e a vista dava
-- TOTAL_FALTAS=5, INJUSTIFICADA.
--
-- Alteração: a condição passa a exigir NVL(s.FALTA,0) = 1. TOTAL_FALTAS_PENDENTES e
-- TOTAL_FALTAS_JUSTIFICADAS não precisam do filtro — já dependem de existir uma RH_T_FALTA.
--
-- Backup do texto anterior: docs/sql/rh_v_resumo_assiduidade_BACKUP_11-09.sql
...
WITH resumo AS (
    SELECT
        ...
        COUNT(
            CASE
                WHEN NVL(s.FALTA, 0) = 1 AND f2.id IS NULL AND aus.id IS NULL
                    THEN 1
            END
        ) AS total_faltas,
