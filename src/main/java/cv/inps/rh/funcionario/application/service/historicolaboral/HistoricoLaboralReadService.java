...
public class HistoricoLaboralReadService {
    ...
    public WrapperHistLaboralResponseDTO getHistoricoLaboral(GetHistoricoLaboralQuery query) {
        ...
        var data = page.stream()
                .map(obj -> {
                    ...
                    ofNullable(obj.getContrVinculoId().getVinculoId()).map(ParamVinculoEntity::getNome)
                            .ifPresent(response::setVinculo);
                    ofNullable(obj.getSeccaoId()).map(SecaoEntity::getInstId).map(InstituicaoEntity::getNome)
                            .ifPresent(response::setDirecao);
                    ofNullable(obj.getSeccaoId()).map(SecaoEntity::getNome).ifPresent(response::setSeccao);
                    ofNullable(obj.getCarreiraId()).map(CarreiraEntity::getCarrPccsId).map(ParamCarreiraEntity::getNome)
                    ...
                    var dataInicioContrato = obj.getContrVinculoId().getDataInicio() != null
                            ? DateFormatter.localDateToString(obj.getContrVinculoId().getDataInicio())
                            : StringUtils.EMPTY;
                    var dataFimContrato = obj.getContrVinculoId().getDataFim() != null
                            ? DateFormatter.localDateToString(obj.getContrVinculoId().getDataFim())
                            : StringUtils.EMPTY;

                    var dataInicioCarreira = obj.getCarreiraId() != null
                            ? DateFormatter.localDateToString(obj.getCarreiraId().getDataInicio())
                            : StringUtils.EMPTY;
                    var dataFimCarreira = obj.getCarreiraId() != null
                            ? DateFormatter.localDateToString(obj.getCarreiraId().getDataFim())
                            : StringUtils.EMPTY;
                    ...
                    var end = obj.getDataFim() != null ? " / " + DateFormatter.localDateToString(obj.getDataFim())
                            : StringUtils.EMPTY;
    ...
    public ValidarNovoHistoricoLaboralDTO getHistoricoLaboralById(GetHistoricoLaboralByIdQuery query) {
        ...
        if (contrato != null) {
            dto.setContrato(contrato.getTpContratoId() != null ? contrato.getTpContratoId().getNome() : null);
