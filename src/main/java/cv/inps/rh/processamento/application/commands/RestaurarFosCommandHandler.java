...
public class RestaurarFosCommandHandler implements CommandHandler<RestaurarFosCommand, ResponseEntity<String>> {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(RestaurarFosCommandHandler.class);

  private final FosService fosService;

  public RestaurarFosCommandHandler(FosService fosService) {
    this.fosService = fosService;
  }

  @IgrpCommandHandler
  public ResponseEntity<String> handle(RestaurarFosCommand command) {
