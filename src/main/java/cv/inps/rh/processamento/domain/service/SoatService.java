...
public class SoatService {
  ...
  public synchronized DadosApoliceResponseDTO salvarDadosApolice(
    ...
    dadosApoliceRepository.findFirstByIlhaIdAndEstadoOrderByIdDesc(
                    request.getIlhaId(), Estado.A.getCode())
            .ifPresent(atual -> {
              atual.setEstado(Estado.I.getCode());
              dadosApoliceRepository.saveAndFlush(atual);
            });

    var novaApolice = new DadosApoliceEntity();
    novaApolice.setUuid(UuidCreator.getTimeOrderedEpoch().toString());
    novaApolice.setEstado(Estado.A.getCode());
    novaApolice.setNumApolice(request.getNumApolice());
    novaApolice.setIlhaId(request.getIlhaId());
    novaApolice.setDataApolice(request.getDataApolice());

    return toResponse(dadosApoliceRepository.save(novaApolice));
  }
  ...
  public SoatPdfResult gerarFicheiroSoat(String soatUuid, String apoliceUuid) {
    ...
    var pdf = pdfGenerator.generate("soat-pdf", Map.of("soat", pdfData));
    var filename = "soat-%d%02d-%s.pdf".formatted(
            soat.getAnoReferente(),
            soat.getMesReferente(),
            safeFilename(apolice.getNumApolice())
    );
    return new SoatPdfResult(filename, pdf);
  }

  private SoatPdfRowDTO toPdfRow(
          cv.inps.rh.shared.infrastructure.persistence.entity.SoatDetalheEntity detalhe) {
  ...
  private String getLogoBase64() {
    try (var input = getClass().getResourceAsStream("/static/img/inps_logo.png")) {
      if (input == null) {
        throw IgrpResponseStatusException.internalServerError("INPS logo not found");
      }
      return "data:image/png;base64," + Base64.getEncoder().encodeToString(input.readAllBytes());
    } catch (IOException exception) {
      throw IgrpResponseStatusException.internalServerError("Unable to load INPS logo");
    }
  }
