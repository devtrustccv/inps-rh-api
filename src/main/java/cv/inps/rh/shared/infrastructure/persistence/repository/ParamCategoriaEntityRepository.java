...
public interface ParamCategoriaEntityRepository extends
  ...
  List<ParamCategoriaEntity> findAllByEstadoAndParamCarrId(Estado estado, ParamCarreiraEntity paramCarrId);

  Optional<ParamCategoriaEntity> findByUuid(UUID uuid);

  default ParamCategoriaEntity findByUuidOrThrow(UUID uuid) {
    return findByUuid(uuid)
        .orElseThrow(() -> IgrpResponseStatusException.notFound("ParamCategoriaEntity not found for id: " + uuid));
  }
}
