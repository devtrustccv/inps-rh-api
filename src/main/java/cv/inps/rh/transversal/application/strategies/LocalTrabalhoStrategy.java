...
@Component
public class LocalTrabalhoStrategy implements DimensaoStrategy {

    @Override
    public DimensaoEnum getNomeDimensao() {
        return DimensaoEnum.LOCAL_TRABALHO;
    }

    @Override
    public List<Selection<?>> getSelectExpressions(Root<TiposRelacionamentoEntity> root, CriteriaBuilder cb) {
        return List.of(
                root.get("mobId").get("localTrabId").get("id").alias("local_trabalho_id"),
                root.get("mobId").get("localTrabId").get("nome").alias("local_trabalho_nome")
        );
    }
    ...
    public List<Expression<?>> getGroupByExpressions(Root<TiposRelacionamentoEntity> root, CriteriaBuilder cb) {
        return List.of(
                root.get("mobId").get("localTrabId").get("id"),
                root.get("mobId").get("localTrabId").get("nome")
        );
    }

    @Override
    public Predicate getFiltroPredicate(Root<TiposRelacionamentoEntity> root, CriteriaBuilder cb, List<String> valores) {
        return root.get("mobId").get("localTrabId").get("id").in(valores);
    }
}
